package org.bitbucket.socialroboticshub.actions.memory;

import eis.iilang.Identifier;
import eis.iilang.Parameter;
import org.bitbucket.socialroboticshub.actions.RobotAction;

import java.util.List;

public class GetMemoryEntryAction extends RobotAction {
    public final static String NAME = "getMemoryEntry";

    /**
     * @param parameters A list of 3 identifiers represent the interactant ID, the entry
     *                   key and the entry id that needs to be retrieved.
     */
    public GetMemoryEntryAction(final List<Parameter> parameters) {
        super(parameters);
    }

    @Override
    public boolean isValid() {
        return (getParameters().size() == 3) && (getParameters().get(0) instanceof Identifier)
                && (getParameters().get(1) instanceof Identifier)
                && (getParameters().get(2) instanceof Identifier);
    }

    @Override
    public String getTopic() {
        return "memory_get_entry";
    }

    @Override
    public String getData() {
        return (EIStoString(getParameters().get(0)) + ";"
                + EIStoString(getParameters().get(1)) + ";"
                + EIStoString(getParameters().get(2)));
    }

    @Override
    public String getExpectedEvent() {
        return null;
    }
}
