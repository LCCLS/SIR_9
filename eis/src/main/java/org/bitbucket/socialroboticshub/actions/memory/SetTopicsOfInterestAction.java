package org.bitbucket.socialroboticshub.actions.memory;

import eis.iilang.Identifier;
import eis.iilang.Parameter;
import eis.iilang.ParameterList;
import org.bitbucket.socialroboticshub.actions.RobotAction;

import java.util.List;

public class SetTopicsOfInterestAction extends RobotAction {
    public final static String NAME = "setTopicsOfInterest";

    /**
     * @param parameters A list of 2 identifiers: a (string) interactant id
     *                   and list of topics.
     */
    public SetTopicsOfInterestAction(final List<Parameter> parameters) {
        super(parameters);
    }

    @Override
    public boolean isValid() {
        return (getParameters().size() == 2) && (getParameters().get(0) instanceof Identifier)
                && (getParameters().get(1) instanceof ParameterList);
    }

    @Override
    public String getTopic() {
        return "memory_set_topics_of_interest";
    }

    @Override
    public String getData() {
        return EIStoString(getParameters().get(0)) + ";" + EIStoString(getParameters().get(1));
    }

    @Override
    public String getExpectedEvent() {
        return "TopicsOfInterestSet";
    }
}
